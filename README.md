...
라우트는 [main.dart](khu_farm/lib/main.dart) 의 `routes` 맵에서 모든 화면을 명시적으로 등록(`/consumer/...`, `/retailer/...`, `/farmer/...`)하여, 역할별 진입점과 권한 경계를 명확히 분리했습니다.

### 2. 인증 / 자동 로그인 / 토큰 갱신
- Access Token(10시간 만료) + Refresh Token 이중 구조
- 앱 부팅 시 [auth_service.dart](khu_farm/lib/services/auth_service.dart) 의 `tryAutoLogin()` → `getValidAccessToken()` → `refreshAccessToken()` 체인으로 무중단 갱신
- 로그인 상태일 때만 FCM 토큰을 서버에 등록하여 비로그인 사용자에게 푸시가 발송되지 않도록 제어 ([main.dart:178](khu_farm/lib/main.dart#L178))
...
### 4. 농가 전용 운영 콘솔
[farmer/mypage/manage](khu_farm/lib/screens/farmer/mypage/manage/) 하위에 입점 농가가 직접 운영할 수 있는 어드민 수준의 화면을 구현했습니다.
...
### 5. 친환경 리워드 — 만보기 연동
[pedometer_service.dart](khu_farm/lib/services/pedometer_service.dart) 에서 다음을 처리합니다.
...
### 6. 지역 날씨 (공공데이터포털 연동)
[weather_service.dart](khu_farm/lib/services/weather_service.dart) 는 기상청 단기예보 API를 사용합니다.
...
### 7. FCM 푸시 + 로컬 알림 채널
[main.dart](khu_farm/lib/main.dart#L154) 에서 안드로이드 알림 채널(`fcm_default_channel`, `Importance.max`)을 명시 생성하고, iOS는 `DarwinInitializationSettings` 로 권한을 분기 요청합니다. 알림 수신 → 앱 상태별(Foreground/Background/Terminated) 분기는 [notifiaction_service.dart](khu_farm/lib/services/notifiaction_service.dart) 에서 일괄 처리합니다.
